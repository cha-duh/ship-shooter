...
public class ShipShooter_FULL extends PApplet {
...
public void drawTransition1() {
  ...
  if (ship.xPos > width*2.5f) level = TUNNEL;
}
...
public void makeExplosion(float x, float y, boolean goodie) {
  for (int i=0; i<30; i++) {
    fill(i*8, i*3, 0);
    ellipse(x, y, i*2, i*2);
  }
...
public void drawStatus(float w) {
  fill(255, 0, 0);
  rectMode(CORNER);
...
class BossBattle {
  ...
  public void drawBattle() {
    ...
    for (int i=0; i<bsize; i++) {
      landscape[0][i] = map(noise(i/100.0f, slowInc, inc), 0, 1, height/2+100, height/2+150);
...
class Particle {
  ...
  Particle(PVector l) {
    ...
    velocity = new PVector(random(-1.5f,1.5f),random(0,1));
    location = l.get();
    lifespan = 80.0f;
    grow = 15;
  }
...
class Ship {
  ...
  public void fly() {
    ...
    if (hasShield) {
      ...
      und=und%TWO_PI;
      stroke(255);
      strokeWeight(map(cos(und), -1, 1, .5f, 1.5f));
      ellipse(xPos, yPos, 65+(10*sin(und)), 70+(10*cos(und)));
    }
    if (hasSuperShield) {
      hasShield=false;
      if (ssCount < 500) {
        for (float inc=0;inc<TWO_PI;inc+=.03f) {
          float shock = random(TWO_PI)*.03f;
...
class TunnelTraverse {
  ...
  public void drawTraversal() {
    ...
    bezier(50*cos(frameCount), 50*sin(frameCount), width/3*cos(frameCount+noise(frameCount/100.9f)), width/3*sin(frameCount+noise(frameCount/234.9f)), 
      2*width/3*cos(frameCount+noise(frameCount/122.4f)), 2*width/3*sin(frameCount+noise(frameCount/314.4f)), width*cos(frameCount+noise(234.9f)), width*sin(frameCount+noise(232.11f)));
    popMatrix();
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "ShipShooter_FULL" };
    if (passedArgs != null) {
